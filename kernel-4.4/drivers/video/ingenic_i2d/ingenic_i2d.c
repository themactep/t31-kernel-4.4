...
static long i2d_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
		...
		case IOCTL_I2D_RES_PBUFF:
			...
			break;
		case IOCTL_I2D_BUF_LOCK:
			ret = wait_for_completion_interruptible_timeout(&i2d->done_buf, msecs_to_jiffies(2000));
			if (ret < 0) {
				printk("i2d: done_buf wait_for_completion_interruptible_timeout err %d\n", ret);
			} else if (ret == 0) {
				printk("i2d: done_buf wait_for_completion_interruptible_timeout timeout %d\n", ret);
				ret = -1;
				i2d_dump_info(i2d);
			} else {
				ret = 0;
