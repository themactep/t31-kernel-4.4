...
static int TS_CODEC_CCR_21_value = 0;
...
static int TS_CODEC_CCR_21_init(void)
{
	if(time_status <= 7) {
		TS_CODEC_CCR_21_value |= TS_CODEC_CCR_21_value << 1 | 0x01;
		codec_reg_set(TS_CODEC_CCR_21, 0, 7, TS_CODEC_CCR_21_value);
...
static int TS_CODEC_CMICGAINR_24_init(void)
{
	if(time_status == 2) {
...
//	msleep(10);
	...
	if(!codec_fast_inited) {
		time_status = 0;
		timer = kzalloc(sizeof(struct timer_list), GFP_KERNEL);
		if (!timer )
		{
			printk("codec : init time file \n");
			return -1;
		}
		init_timer(timer );
		timer->function = TS_CODEC_CCR_21_init;
...
static int codec_enable_left_record(void)
{
	...
	init_timer(timer );
	timer->function = TS_CODEC_CMICGAINR_24_init;
...
int codec_fast_start(void)
{
	codec_init();
	// codec_enable_right_record();
		codec_fast_inited = 1;
}
